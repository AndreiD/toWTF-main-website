<template>
  <div>
    <div
      class="mobile"
      style="display: flex; flex-direction: column; align-items: flex-end"
    >
      <span style="margin: 0 20px">DAO</span>
      <div class="hr"></div>
    </div>
    <div class="desktop" style="height: 30px"></div>
    <div
      class="desktop"
      style="position: absolute; width: 100%; transform: translateY(65px)"
    >
      <div class="container">
        <h4 style="width: 110px !important; direction: rtl">DAO</h4>
      </div>
      <div class="hr desktop" style="z-index: 0"></div>
    </div>
    <div class="container" style="padding-top: 20px; margin: 0 auto 50px">
      <div style="position: relative">
        <div style="display: flex; align-items: flex-end">
          <h1 class="mobile" style="padding: 0 20px">DAO</h1>
          <div class="desktop" style="margin-left: 110px">
            <h1
              style="
                margin-top: 40px;
                padding: 0 50px 0 20px;
                position: relative;
                width: fit-content;
                background-color: #d9dbda;
                z-index: 1;
              "
            >
              DAO
            </h1>
          </div>
        </div>
        <p
          class="alpina desktop"
          style="font-size: 20px; margin: 10px 0 10px 130px; max-width: 540px"
        >
          Many DAOs fall into one of two general buckets: Those that manage open
          source, blockchain-based projects together and those that make
          investments. They can act similar to limited liability companies
          (LLCs), VC firms or investment firms, like PleasrDAO.
          <br /><br />

          But DAOs can be also charities, businesses, social networks, unions or
          political groups.
          <br />
          <br />
          Why DAOs ?
          <br />
          <br />- Inclusive - anyone across the world can join and participate.
          <br />v- Democratic - all decisions may be decided by the community.
          <br />- Transparent - every transaction to and from a DAO is public.
          <br />- Permissionless - DAOs are unregulated and cannot be sanctioned
          by governments.

          <br />Why not DAO

          <br />Can be complicated - A DAO is a new technology so its
          terminology and user experience can be confusing. This is where we
          will help!

          <br />OUR JOB IS:

          <br />- Advice how to build them <br />- Building them <br />-
          Onboarding new members <br />- Risk Management

          <br />Note that:
          <br />
          <br />usual time to deliver 7-14 days (and up to 1 month for
          onboarding) <br />100% refund if we don't meet the deadline
        </p>
        <p class="alpina mobile" style="font-size: 20px; margin: 0 20px">
          Many DAOs fall into one of two general buckets: Those that manage open
          source, blockchain-based projects together and those that make
          investments. They can act similar to limited liability companies
          (LLCs), VC firms or investment firms, like PleasrDAO.
          <br /><br />

          But DAOs can be also charities, businesses, social networks, unions or
          political groups.
          <br />
          <br />
          Why DAOs ?
          <br />
          <br />- Inclusive - anyone across the world can join and participate.
          <br />v- Democratic - all decisions may be decided by the community.
          <br />- Transparent - every transaction to and from a DAO is public.
          <br />- Permissionless - DAOs are unregulated and cannot be sanctioned
          by governments.

          <br />Why not DAO

          <br />Can be complicated - A DAO is a new technology so its
          terminology and user experience can be confusing. This is where we
          will help!

          <br />OUR JOB IS:

          <br />- Advice how to build them <br />- Building them <br />-
          Onboarding new members <br />- Risk Management

          <br />Note that:
          <br />
          <br />usual time to deliver 7-14 days (and up to 1 month for
          onboarding) <br />100% refund if we don't meet the deadline
        </p>
      </div>
    </div>
  </div>
</template>
